...
def load_dataset(name: str, folder: Union[Path, str] = dataset_dir, time_scale=1.0):
    ...
    sequences = [
        Sequence(
            ...
            t_end=seq.get("t_end")* time_scale,#上面这个start大家一般都为0，只是这个t_end不一定啊，有不一定的。
...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def get_dataloader(
            self, batch_size: int = 32, shuffle: bool = True, model_index= 0
    ...
    def get_inter_time_max(self):#我们第一次，就直接平均分好了，所以我们只需要一个max即可。
        """Get the mean and std of log(inter_time)."""
        all_inter_times = torch.cat([seq.inter_times[:-1] for seq in self.sequences])#这个是指代不包括那个T的在一起。
